...
class func:
    ...
    def release_brake():
        aks_communication.send("b0\n")
        time.sleep(0.5)
        brake=0
        
    def apply_brake():
        aks_communication.send("b1\n")
        time.sleep(0.5)
        brake=1
    
    def straighten_wheels():
        global steeringAngle
        aks_communication.send("t0\n")
        steeringAngle=0
        time.sleep(0.5)
        
    def put_reverse():
        global gear
        aks_communication.send("r\n")
        time.sleep(0.2)
        gear=1
    ...
    def turn_right():
        ...
        if steeringAngle>-95:
            ...
            aks_communication.send(steeringAngle) 
        else:
            pass
                ...
                while start:
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.JOYAXISMOTION:
                            if event.axis == 0:  # x ekseni
                                if event.value < -0.5:
                                    func.turn_left()
                                elif event.value > 0.5:
                                    func.turn_right()
                            elif event.axis == 1:  # y ekseni
                                if event.value < -0.5:
                                    func.go()
                                elif event.value > 0.5:
                                    func.go_back()
